/**
 * Census Bureau ACS5 data service — Workers-compatible.
 * Uses Supabase JS for caching instead of Drizzle storage.
 */
...
const SPA_MAPPING: Record<string, { description: string; approximateArea: string }> = {
  "SPA 1": { description: "Antelope Valley",      approximateArea: "Lancaster/Palmdale area" },
  "SPA 2": { description: "San Fernando Valley",  approximateArea: "San Fernando Valley area" },
  "SPA 3": { description: "San Gabriel Valley",   approximateArea: "Pasadena/San Gabriel area" },
  "SPA 4": { description: "Metro LA",             approximateArea: "Downtown LA/Hollywood area" },
  "SPA 5": { description: "West LA",              approximateArea: "Santa Monica/Beverly Hills area" },
  "SPA 6": { description: "South LA",             approximateArea: "South Los Angeles/Compton area" },
  "SPA 7": { description: "East LA",              approximateArea: "East Los Angeles/Whittier area" },
  "SPA 8": { description: "South Bay/Harbor",     approximateArea: "Long Beach/Torrance area" },
};
...
async function fetchFromCensus(censusApiKey: string | undefined, variables: string, forGeo: string, inGeo?: string): Promise<string[][] | null> {
  try {
    let url = `${CENSUS_BASE}?get=${variables}&for=${forGeo}`;
    if (inGeo) url += `&in=${inGeo}`;
    if (censusApiKey) url += `&key=${censusApiKey}`;
...
async function getCensusCache(supabase: SupabaseClient, level: string, value: string) {
  const { data } = await supabase
    .from("census_cache")
    .select("*")
    .eq("geography_level", level)
    .eq("geography_value", value)
    .maybeSingle();
  return data;
}

async function upsertCensusCache(
  ...
  const existing = await getCensusCache(supabase, data.geography_level, data.geography_value);
  if (existing) {
    await supabase
      .from("census_cache")
    ...
    await supabase.from("census_cache").insert(data);
  }
}

// ─── Census fetchers ──────────────────────────────────────────────────────────

async function fetchStateCensus(supabase: SupabaseClient, censusApiKey: string | undefined, stateName: string): Promise<CensusComparison | null> {
  const fips = findStateFips(stateName);
  if (!fips) return null;

  const cached = await getCensusCache(supabase, "State", stateName);
  if (cached && cached.fetched_at && (Date.now() - new Date(cached.fetched_at).getTime()) < CACHE_TTL_MS) {
    return {
      geographyLevel: "State", geographyValue: stateName,
      totalPopulation: cached.total_population,
      povertyRate: cached.poverty_count && cached.poverty_universe
        ? Math.round((cached.poverty_count / cached.poverty_universe) * 1000) / 10 : null,
      medianIncome: cached.median_income,
      isApproximate: false, dataYear: cached.data_year,
    };
  }

  const data = await fetchFromCensus(censusApiKey, "NAME,B01003_001E,B17001_002E,B17001_001E,B19013_001E", `state:${fips}`);
  ...
  await upsertCensusCache(supabase, {
    geography_level: "State", geography_value: stateName, state_code: fips,
    total_population: totalPop, poverty_count: povertyCt, poverty_universe: povertyUni,
    median_income: medIncome, data_year: DATA_YEAR,
  });

  return {
    geographyLevel: "State", geographyValue: stateName, totalPopulation: totalPop,
    povertyRate: povertyCt && povertyUni ? Math.round((povertyCt / povertyUni) * 1000) / 10 : null,
    medianIncome: medIncome, isApproximate: false, dataYear: DATA_YEAR,
  };
}

async function fetchCountyCensus(supabase: SupabaseClient, censusApiKey: string | undefined, countyName: string): Promise<CensusComparison | null> {
  const cached = await getCensusCache(supabase, "County", countyName);
  if (cached && cached.fetched_at && (Date.now() - new Date(cached.fetched_at).getTime()) < CACHE_TTL_MS) {
    return {
      geographyLevel: "County", geographyValue: countyName,
      totalPopulation: cached.total_population,
      povertyRate: cached.poverty_count && cached.poverty_universe
        ? Math.round((cached.poverty_count / cached.poverty_universe) * 1000) / 10 : null,
      medianIncome: cached.median_income,
  ...
  const data = await fetchFromCensus(censusApiKey, "NAME,B01003_001E,B17001_002E,B17001_001E,B19013_001E", "county:*", "state:*");
  if (!data || data.length < 2) return null;
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (!fallback && name.includes(normalized)) fallback = data[i];
  }
  ...
  await upsertCensusCache(supabase, {
    geography_level: "County", geography_value: countyName, state_code: stateCode || null,
    total_population: totalPop, poverty_count: povertyCt, poverty_universe: povertyUni,
    median_income: medIncome, data_year: DATA_YEAR,
  });

  return {
    geographyLevel: "County", geographyValue: countyName, totalPopulation: totalPop,
    povertyRate: povertyCt && povertyUni ? Math.round((povertyCt / povertyUni) * 1000) / 10 : null,
    medianIncome: medIncome, isApproximate: false, dataYear: DATA_YEAR,
  };
}

async function fetchCityCensus(supabase: SupabaseClient, censusApiKey: string | undefined, cityName: string): Promise<CensusComparison | null> {
  const cached = await getCensusCache(supabase, "City", cityName);
  if (cached && cached.fetched_at && (Date.now() - new Date(cached.fetched_at).getTime()) < CACHE_TTL_MS) {
    return {
      geographyLevel: "City", geographyValue: cityName,
      totalPopulation: cached.total_population,
      povertyRate: cached.poverty_count && cached.poverty_universe
        ? Math.round((cached.poverty_count / cached.poverty_universe) * 1000) / 10 : null,
      medianIncome: cached.median_income,
  ...
  for (const stateCode of ["06", "36", "48", "12", "17", ...allStates]) {
    const data = await fetchFromCensus(censusApiKey, "NAME,B01003_001E,B17001_002E,B17001_001E,B19013_001E", "place:*", `state:${stateCode}`);
    ...
    for (let i = 1; i < data.length; i++) {
      const name = (data[i][0] || "").toLowerCase();
      if (name.startsWith(exactPattern)) { bestMatch = data[i]; bestScore = 100; break; }
      if (name.startsWith(cdpPattern)   && bestScore < 80) { bestMatch = data[i]; bestScore = 80; }
      if (name.startsWith(townPattern)  && bestScore < 70) { bestMatch = data[i]; bestScore = 70; }
      if (name.includes(normalized)     && bestScore < 10) { bestMatch = data[i]; bestScore = 10; }
    }
    if (bestMatch) { matchRow = bestMatch; break; }
  }
  ...
  await upsertCensusCache(supabase, {
    geography_level: "City", geography_value: cityName, state_code: null,
    total_population: totalPop, poverty_count: povertyCt, poverty_universe: povertyUni,
    median_income: medIncome, data_year: DATA_YEAR,
  });

  return {
    geographyLevel: "City", geographyValue: cityName, totalPopulation: totalPop,
    povertyRate: povertyCt && povertyUni ? Math.round((povertyCt / povertyUni) * 1000) / 10 : null,
    medianIncome: medIncome, isApproximate: false, dataYear: DATA_YEAR,
  };
}
...
function getSPACensus(spaValue: string): CensusComparison {
  ...
  return {
    geographyLevel: "SPA", geographyValue: spaValue,
...
export async function getCensusComparison(
  ...
  try {
    switch (geographyLevel) {
      case "State": {
        const r = await fetchStateCensus(supabase, censusApiKey, geographyValue);
        if (r) return r;
        break;
      }
      case "County": {
        const r = await fetchCountyCensus(supabase, censusApiKey, geographyValue);
        if (r) return r;
        break;
      }
      case "City": {
        const r = await fetchCityCensus(supabase, censusApiKey, geographyValue);
        if (r) return r;
        break;
      }
      case "SPA":
  ...
  return { geographyLevel, geographyValue, totalPopulation: null, povertyRate: null, medianIncome: null, isApproximate: false, dataYear: DATA_YEAR };
}
...
export async function getCensusForGeographies(
  ...
  return Promise.all(Array.from(unique.values()).map(g => getCensusComparison(supabase, censusApiKey, g.level, g.value)));
}
...
async function fetchAgeDataForGeography(
  ...
  if (geographyLevel === "SPA") {
    ...
    if (!spaData || !ageDist) {
      return { geographyLevel, geographyValue, totalPopulation: null, targetAgePopulation: null, ageGroups: [], isApproximate: true, dataYear: DATA_YEAR };
    }
    ...
    return { geographyLevel, geographyValue, totalPopulation: spaData.totalPopulation, targetAgePopulation: null, ageGroups, isApproximate: true, dataYear: DATA_YEAR };
  }
  ...
  if (geographyLevel === "State") {
    ...
    if (fips) data = await fetchFromCensus(censusApiKey, varString, `state:${fips}`);
  } else if (geographyLevel === "County") {
    data = await fetchFromCensus(censusApiKey, varString, "county:*", "state:*");
    if (data && data.length >= 2) {
      const normalized = geographyValue.toLowerCase().replace(/\s*county\s*/i, "").trim();
      const exactPattern = `${normalized} county,`;
      let matchRow: string[] | undefined;
      for (let i = 1; i < data.length; i++) {
        if ((data[i][0] || "").toLowerCase().startsWith(exactPattern)) { matchRow = data[i]; break; }
      }
    ...
    const allStates = Object.values(STATE_FIPS);
    for (const stateCode of ["06", "36", "48", "12", "17", ...allStates]) {
      const stateData = await fetchFromCensus(censusApiKey, varString, "place:*", `state:${stateCode}`);
  ...
  if (!data || data.length < 2) {
    return { geographyLevel, geographyValue, totalPopulation: null, targetAgePopulation: null, ageGroups: [], isApproximate: false, dataYear: DATA_YEAR };
  }
  ...
  return { geographyLevel, geographyValue, totalPopulation: totalPop, targetAgePopulation: null, ageGroups, isApproximate: false, dataYear: DATA_YEAR };
}
...
export async function getCensusAgeGroups(
  ...
  const results = await Promise.all(
    Array.from(unique.values()).map(g => fetchAgeDataForGeography(supabase, censusApiKey, g.level, g.value))
  );
